...
export async function getSlaStats(req, res) {
  try {
    ...
    SELECT
        json_build_object(
            'threshold_hours', (SELECT threshold FROM params),
            'total_open', (SELECT COUNT(*) FROM open_posts),
            'overdue_count', (SELECT COUNT(*) FROM open_posts WHERE hours_open > (SELECT threshold FROM params)),
            'posts', (
                SELECT COALESCE(json_agg(
                    json_build_object(
                        ...
                        'overdue', (hours_open > (SELECT threshold FROM params)),
